...
# Напишем функцию для быстрого обзора датафрейма
...
def show_2_plots(df, df2, feature, type='hist', new_bins=0, new_hue=None, 
                ...
                main_title=None, new_stat='count', new_common_norm=True, new_kde=True,
                xscale=None, yscale=None, show_outliers=True):
    """
    ...
    if type=='hist':
        ...
        ax1.set_title(f'Диаграмма частот')
        ax1.set_xlabel(feature)
        ax1.set_ylabel('Количество')
    ...
    sns.boxplot(x=df[feature].dropna(), 
                ...
                width=0.2,
                ax=ax2)
    ax2.set_title(f'Диаграмма размаха. {df_label}')
    ax2.set_xlabel(feature)
    ...
    sns.boxplot(x=df2[feature].dropna(), 
                ...
                width=0.2,
                ax=ax3)
    ax3.set_title(f'Диаграмма размаха. {df2_label}')
...
# Функция для вывода текстового описания уровней корреляции для таргета
def print_corr_levels_target(corr_matrix, target='Договор расторгнут', threshold=0.0):
    corr_list = []
    ...
    for row in corr_matrix.index:
        if row == target:
            continue
        corr_value = corr_matrix.loc[row, target]
        if abs(corr_value) > 0.9 and abs(corr_value) > threshold:
            corr_list.append((f"Весьма высокая корреляция между {target} и {row}:", round(corr_value, 3)))
        elif abs(corr_value) > 0.7 and abs(corr_value) > threshold:
            corr_list.append((f"Высокая корреляция между {target} и {row}:", round(corr_value, 3)))
        elif abs(corr_value) > 0.5 and abs(corr_value) > threshold:
            corr_list.append((f"Заметная корреляция между {target} и {row}:", round(corr_value, 3)))
        elif abs(corr_value) > 0.3 and abs(corr_value) > threshold:
            corr_list.append((f"Умеренная корреляция между {target} и {row}:", round(corr_value, 3)))
        elif abs(corr_value) > 0.1 and abs(corr_value) > threshold:
            corr_list.append((f"Слабая корреляция между {target} и {row}:", round(corr_value, 3)))
        elif abs(corr_value) > 0.0 and abs(corr_value) > threshold:
            corr_list.append((f"околонулевая корреляция между {target} и {row}:", round(corr_value, 3)))
